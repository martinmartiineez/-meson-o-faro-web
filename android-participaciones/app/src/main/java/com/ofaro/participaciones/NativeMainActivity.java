...
public class NativeMainActivity extends Activity {
    ...
    private android.view.View build(){
        ...
        Button reservations=big("RESERVAS","Crear, confirmar, editar, servicio y tickets");reservations.setOnClickListener(v->startActivity(new Intent(this,ReservationsActivity.class)));page.addView(reservations,top(dp(10)));
        ...
        LinearLayout r=row();Button image=tile("SOLO IMAGEN");image.setOnClickListener(v->startActivity(new Intent(this,TicketPreviewActivity.class).putExtra("type","Imagen")));Button tools=tile("QR / PLANTILLAS / HISTORIAL");tools.setOnClickListener(v->startActivity(new Intent(this,ManagementActivityV3.class)));r.addView(image,weight());r.addView(tools,weight());page.addView(r,topWrap(dp(9)));
        Button settings=big("IMPRESORA Y AJUSTES","IP, puerto, conexión permanente, prueba ESC/POS y Apps Script");settings.setOnClickListener(v->startActivity(new Intent(this,PrinterSettingsActivity.class)));page.addView(settings,top(dp(10)));
        ...
        TextView note=txt("La impresión de la APK es siempre LOCAL y DIRECTA a la impresora configurada. La cola 'Enviar a Android' no forma parte del flujo principal.",13,Color.DKGRAY,false);note.setPadding(dp(2),dp(18),dp(2),0);page.addView(note);
        return root;
    }
