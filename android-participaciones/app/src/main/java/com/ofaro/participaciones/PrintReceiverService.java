        ...
        while (running) {
            try {
                ...
                if (now - heartbeatAt > 30000) {
                    JSONObject hb=core.action("printQueueHeartbeat").put("appVersion","3.1.1").put("printerIp",core.printerIp());
                    try{core.ensureOk(core.postTo(ApiEndpoint.URL,hb));}catch(Exception ignored){}
                    heartbeatAt=now;
                }
                JSONObject req=core.action("printQueueNext").put("appVersion","3.1.1").put("printerIp",core.printerIp());
                JSONObject res=core.postTo(ApiEndpoint.URL,req);core.ensureOk(res);
                ...
                JSONObject done=core.action("printQueueComplete").put("id",id).put("success",ok).put("error",error).put("appVersion","3.1.1");
                try{core.postTo(ApiEndpoint.URL,done);}catch(Exception ignored){}
