...
public class InternalAlertsActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle state){
        super.onCreate(state);V5Ui.applySystemBars(this);center=new InternalAlertCenter(this);render();
    }

    private void render(){
        ScrollView scroll=new ScrollView(this);scroll.setFillViewport(true);root=V5Ui.column(this);root.setPadding(V5Ui.dp(this,18),V5Ui.dp(this,12),V5Ui.dp(this,18),V5Ui.dp(this,28));scroll.addView(root);setContentView(scroll);
        ...
        root.addView(V5Ui.kicker(this,"ALERTAS INTERNAS"));TextView title=V5Ui.title(this,"Centro de avisos");title.setPadding(0,V5Ui.dp(this,5),0,V5Ui.dp(this,4));root.addView(title);
        root.addView(V5Ui.subtitle(this,"Reservas, impresora y stock de premios sin perder de vista el servicio."));

        int unread=center.unreadCount();LinearLayout summary=V5Ui.darkCard(this);summary.addView(V5Ui.text(this,String.valueOf(unread),30,android.graphics.Color.WHITE,true));summary.addView(V5Ui.text(this,unread==1?"alerta sin leer":"alertas sin leer",10.5f,android.graphics.Color.rgb(195,207,197),false));root.addView(summary,top(12));
        ...
        Spinner lead=spinner(new String[]{"15 min","30 min","45 min","60 min"});int current=center.prefs().getInt("alert_lead_minutes",30);lead.setSelection(current<=15?0:current<=30?1:current<=45?2:3);lead.setOnItemSelectedListener(new SimpleItemListener(pos->center.prefs().edit().putInt("alert_lead_minutes",new int[]{15,30,45,60}[pos]).apply()));root.addView(lead,new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,V5Ui.dp(this,48)));

        root.addView(section("STOCK DE PREMIOS"));
        Spinner stock=spinner(new String[]{"1 unidad","2 unidades","3 unidades","5 unidades","10 unidades"});int threshold=center.prefs().getInt("alert_stock_threshold",3);int stockPos=threshold<=1?0:threshold<=2?1:threshold<=3?2:threshold<=5?3:4;stock.setSelection(stockPos);stock.setOnItemSelectedListener(new SimpleItemListener(pos->center.prefs().edit().putInt("alert_stock_threshold",new int[]{1,2,3,5,10}[pos]).apply()));root.addView(stock,new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,V5Ui.dp(this,48)));

        LinearLayout buttons=V5Ui.row(this);buttons.addView(button("Marcar leídas",false,()->{center.markAllRead();render();}),weight());buttons.addView(button("Borrar historial",false,this::confirmClear),weightGap());root.addView(buttons,top(12));

        root.addView(section("HISTORIAL"));JSONArray alerts=center.alerts();if(alerts.length()==0){LinearLayout empty=V5Ui.softCard(this);empty.addView(V5Ui.text(this,"Todavía no hay alertas registradas.",12,V5Ui.MUTED,false));root.addView(empty);}
    ...
    private void addToggle(String title,String key,String note){
        LinearLayout card=V5Ui.card(this);card.setBackground(V5Ui.bg(this,V5Ui.SURFACE_ALT,16));CheckBox box=new CheckBox(this);box.setText(title);box.setTextSize(12.5f);box.setTextColor(V5Ui.INK);box.setChecked(center.prefs().getBoolean(key,true));box.setOnCheckedChangeListener((b,on)->center.prefs().edit().putBoolean(key,on).apply());card.addView(box);card.addView(V5Ui.text(this,note,10,V5Ui.MUTED,false));root.addView(card,bottom(7));
    }

    private LinearLayout alertCard(JSONObject a){
        boolean read=a.optBoolean("read",false);LinearLayout card=V5Ui.card(this);card.setBackground(V5Ui.bg(this,read?V5Ui.SURFACE_ALT:V5Ui.LIME_SOFT,17));
        LinearLayout top=V5Ui.row(this);top.setGravity(Gravity.CENTER_VERTICAL);top.addView(V5Ui.text(this,a.optString("title","Alerta"),13.5f,V5Ui.INK,true),new LinearLayout.LayoutParams(0,ViewGroup.LayoutParams.WRAP_CONTENT,1));top.addView(V5Ui.pill(this,typeLabel(a.optString("type","")),read?android.graphics.Color.rgb(235,236,232):V5Ui.SURFACE,read?V5Ui.MUTED:V5Ui.GREEN));card.addView(top);
        card.addView(V5Ui.text(this,a.optString("message",""),10.5f,V5Ui.MUTED,false));card.addView(V5Ui.text(this,formatTime(a.optLong("ts",0)),9.5f,V5Ui.FAINT,false));return card;
    }
